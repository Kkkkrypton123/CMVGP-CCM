...
class GP():
    ...
    def testStateSpaceCorrelation(self, X, Y, m=3, tau=1, cuda=0):
        ...
        if not X is torch.Tensor:
            for i in range(size):
                ...
                Y[i] = torch.from_numpy(Y[i]).float().cuda(cuda)
        ...
        for i in range(size):
            ...
            yar.append(torch.stack([y[i][j:j - tau * m] for j in range(m)]).cuda(cuda).T)
            yar[i] = (yar[i] - yar[i].mean(0)) / yar[i].std(0)
        ...
        for i in range(niter):
            ...
            for k in range(mSamples):
                ...
                try:
                    ...
                    VV = [torch.linalg.svd(A[i])[-1] for i in range(len(A))]
                except torch.linalg.LinAlgError as e:
                    print("SVD decomposition failed:", e)
                A_copy = [A[a].clone() for a in range(len(A))]  # Create A copy of A
                for a in range(len(A_copy)):
                    A_copy[a] = A_copy[a] + torch.eye(A_copy[a].shape[0]) * 1e-5
                V = [torch.linalg.svd(A_copy[i])[-1] for i in range(len(A_copy))]
                inp = [ip[i] @ V[i] for i in range(len(ip))]
                ripoints = [(ipoints[i] + (sipoints[i] * torch.randn(*ipoints[i].shape).cuda(self.cuda) * std_factor)) for i in range(len(ipoints))]
                rld = [(ld[i] + (sld[i] * torch.randn(*ld[i].shape).cuda(self.cuda) * std_factor)).exp() for i in range(len(ld))]
                rsigma_l = [(sigma_l[i] + (ssigma_l[i] * torch.randn(1).cuda(self.cuda) * std_factor)).exp() for i in range(len(sigma_l))]
                rsigma = [(sigma[i] + (ssigma[i] * torch.randn(1).cuda(self.cuda) * std_factor)).exp() for i in range(len(sigma))]
            ...
            for i in range(len(parms)):
                torch.nn.utils.clip_grad_norm_(parms[i], 1200)
            ...
            with torch.no_grad():
                for i in range(len(sigma)):
                    ...
                    hnoise[i] += 10000 * lr * hnoise[i].grad
    ...
    def forward(self, inp, observations, test, testo,  ld=[[1],[1],[1]], sig=10, noise=[[.1],[.1],[.1]], target="squaredexp"):
        ...
        for a in range(30):
            ...
            # sigma_o = [(self.sigma_o[i] + (self.ssigma_o[i] * np.random.randn() * t)).exp() for i in range(len(self.sigma_o))]
            ...
            # Covariance update
            for j in range(len(posteriorK)):
                posteriorK[j].append([(tk[j] - inptk[j].T.matmul(stdMat[j]).matmul(inptk[j]) + self.noise[j] * eye[j]).slogdet()[1].cpu()])
